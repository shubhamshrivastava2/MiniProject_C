    ...
    if(fptr==NULL){
        ...
        while(fread(book_to_find, sizeof(book_data), 1, fptr)!='\0'){
            if(id==book_to_find->book_id){
                ...
                printf("Date of issue: %d/%d/%d\n", book_to_find->date_of_issue[0], book_to_find->date_of_issue[1], book_to_find->date_of_issue[2]);
                printf("Due date: %d/%d/%d\n", book_to_find->due_date[0], book_to_find->due_date[1], book_to_find->due_date[2]);
                fclose(fptr);
                free(book_to_find);
                return pass;
            }
